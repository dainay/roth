...
export default function Bath({ showVipanelLeft, showVipanelRight, paroi, metalColor, vipanel, receveur, ...props }) {
    ...
    const receveurTexture = useMemo(() => {
        console.log('receveur', receveur)
        return texturesReceveur[Number(receveur) - 1]
    }, [texturesReceveur, receveur])

    useLayoutEffect(() => {
        const material = materials['Receveur.001']

        if (!receveurTexture || !material || !normalMap) return
